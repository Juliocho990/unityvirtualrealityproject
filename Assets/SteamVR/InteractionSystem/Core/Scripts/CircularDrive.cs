...
namespace Valve.VR.InteractionSystem
{
	...
	public class CircularDrive : MonoBehaviour
	{
		...
		private float minMaxAngularThreshold = 10.0f;//1
		...
		void Start()
		{
			...
			if ( transform.parent )
			{
				worldPlaneNormal = transform.parent.localToWorldMatrix.MultiplyVector( worldPlaneNormal ).normalized; // Conivierte el punto 
			}
		...
		private void ComputeAngle( Hand hand )
		{
			...
			if ( !toHandProjected.Equals( lastHandProjected ) )
			{
				...
				if ( absAngleDelta > 0.0f )
				{
					...
					{
						//Como el angulo se le saca el signo
						Vector3 cross = Vector3.Cross( lastHandProjected, toHandProjected ).normalized; //no conmutativo el producto cruz(AxB!=BxA)
