...
namespace Valve.VR.InteractionSystem
{
	...
	public class TeleportPoint : TeleportMarkerBase
	{
		...
		public UnityEvent onHighlightTeleportZone;
		public UnityEvent onRealeaseTeleportZone;
		...
		public override void Highlight( bool highlight )
		{
			...
			if ( highlight )
			{
				if(pointIcon!= moveLocationIcon)
					pointIcon.gameObject.SetActive( true );
				else 
					pointIcon.gameObject.SetActive( false );
				onHighlightTeleportZone.Invoke();
				if(animation.clip.name != moveLocationAnimation)
					animation.Play();
			}
			else
			{
				pointIcon.gameObject.SetActive( false );
				onRealeaseTeleportZone.Invoke ();
				animation.Stop();
			}
		}
